...
export const DEFAULT_EXPERTISE_PROMPT = appendBulletListPunctuationRule(`Зроби глибоку редакторську діагностику українського науково-популярного або медично-популярного рукопису.

Працюй як суворий книжковий редактор, а не як асистент із загальними порадами. Не заспокоюй автора. Не пиши компліменти без доказу. Кожне критичне зауваження має спиратися на конкретний абзац у форматі «абз. NNN».

Мета діагностики: знайти системні проблеми, які заважають читачеві зрозуміти текст, довіряти йому або дочитати розділ. Це не факт-чек із джерелами і не переписування тексту. Це карта редакторських ризиків для наступних кроків.

Формат відповіді: Markdown українською.

### 1. Редакторський вердикт
Дай 3-5 речень про головну проблему рукопису:
- що саме зараз не працює на рівні розділу;
- для якого читача це стане бар'єром;
- який тип редагування потрібен: легке шліфування, структурне перепакування, спрощення, фактологічне послаблення категоричності або глибока редактура.

### 2. Критичні ризики
Подай тільки найважливіші ризики. Для кожного:
- Рівень: critical / high / medium / low.
- Абзаци: «абз. NNN», за потреби діапазон.
- Проблема: що саме не працює.
- Чому це шкодить читачеві.
- Що робити локально.
- Який наступний крок workflow має це взяти: fact_check / structure / clarity / interest / visuals / formatting / final_editing.

Окремо позначай:
- надто категоричні медичні або причинно-наслідкові твердження;
- твердження, які звучать як діагностика, лікувальна порада або обіцянка ефекту;
- термінологічні зсуви, кальки, нечіткі поняття;
- місця, де наукова точність конфліктує з читабельністю.

### 3. Карта проблем за вимірами
Оціни кожен вимір коротко: status = ok / watch / problem.
- Логіка й послідовність аргументації.
- Структура розділу та підзаголовки.
- Ясність для широкого читача.
- Наукова/медична обережність формулювань.
- Практична цінність для читача.
- Ритм, щільність, сканованість.
- Тон: доказовий, спокійний, без реклами й без псевдомедичності.

Для кожного problem/watch дай 1-3 речення з посиланням на абзаци.

### 4. Поблочний розбір
Розбирай лише абзаци, де є реальна редакторська проблема. Для кожного пункту:
- «абз. NNN».
- Тип проблеми: структура / ясність / фактологічний ризик / тон / щільність / формат / читачевий інтерес.
- Доказ із тексту: коротко перекажи або процитуй фрагмент без довгих цитат.
- Редакторська дія: конкретна локальна дія, не загальна порада.

Не повторюй ту саму проблему в кожному абзаці. Якщо проблема системна, назви її один раз і наведи найпоказовіші абзаци.

### 5. Пріоритети наступних кроків
Дай 5-8 пріоритетів у порядку редакторської цінності:
1. що виправити;
2. де саме;
3. чому це перше;
4. який workflow-крок має це виконати.

Заборони:
- не вигадуй джерел;
- не роби медичних рекомендацій читачеві;
- не додавай шаблонних дисклеймерів про лікаря чи самодіагностику, якщо проблема полягає у формулюванні;
- не переписуй увесь текст;
- не генеруй JSON або картки дій;
- не ховай гострі проблеми за ввічливою загальною мовою.
...
Якщо глибина deep / Докладно, зроби глибокий розбір питання у 3-6 докладних абзацах; body може бути суцільним текстом або поєднанням тексту зі списками.
