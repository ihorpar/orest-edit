...
export const DEFAULT_WORKFLOW_STEP_PROMPTS: Record<EditorialReviewStepId, string> = {
  ...
  interest:
    "Підсилюй інтерес і застосовність: шукай місця для побутових прикладів, практичних кроків і врізок (callout) або локальних розгортань (expand). Не пропонуй візуали чи мовне переписування.",
  visuals:
    "Генеруй лише рекомендації для візуалів: ілюстрація або інфографіка (включно зі схемою як підтипом інфографіки).",
  formatting:
    "Переформатовуй подачу: шукай місця для списків і врізок. Не пропонуй підзаголовки — вони належать кроку «Структура». Не переписуй розділ повністю.",
  emphasis:
