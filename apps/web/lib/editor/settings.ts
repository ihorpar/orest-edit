...
export const DEFAULT_CALLOUT_PROMPT_TEMPLATE = `Створи чернетку врізки для українського науково-популярного рукопису. Використай тип: {{calloutKindLabel}}. Під "врізкою" тут мається на увазі додатковий пояснювальний блок, який може бути коротким фактом, мініісторією, поясненням механізму, покроковим розбором або блоком "міф і факт". Спирайся тільки на фрагмент і редакторську рекомендацію, не додавай нових фактів поза текстом. Поверни короткий заголовок, сам текст врізки і стисле пояснення, навіщо вона тут. Контекст фрагмента: {{fragment}}. Рекомендація: {{recommendation}}.`;
export const DEFAULT_IMAGE_PROMPT_TEMPLATE = `Підготуй prompt для генерації чернеткової ілюстрації для книжкового редактора. Мета: допомогти ілюстратору швидко зрозуміти, що саме варто візуалізувати у фрагменті. Стиль обов'язково: мінімалістичний, простий, чернетка для ілюстратора. Вкажи: що саме показати, яку освітню функцію має виконати візуал, які елементи обов'язкові, яких візуальних кліше або зайвого декору уникати. Тип візуалу: {{visualIntent}}. Фрагмент: {{fragment}}. Рекомендація: {{recommendation}}. Мова фінального prompt: тільки українська, без англіцизмів.`;
