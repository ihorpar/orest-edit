...
export const DEFAULT_CALLOUT_PROMPT_TEMPLATE = `Створи чернетку врізки для українського науково-популярного рукопису. Використай тип: {{calloutKindLabel}}. Дозволені режими врізки: mechanism, analogy, everyday_application, myths_vs_truth, top_list. Спирайся тільки на фрагмент і редакторську рекомендацію, не додавай нових фактів поза текстом. Поверни короткий заголовок, сам текст врізки і стисле пояснення, навіщо вона тут. Контекст фрагмента: {{fragment}}. Рекомендація: {{recommendation}}.`;
export const DEFAULT_IMAGE_PROMPT_TEMPLATE = `Підготуй український prompt для генерації чернеткового візуалу для книжкового редактора. Мета: допомогти швидко зрозуміти, що саме треба показати у фрагменті. Вкажи: що саме показати, яку освітню функцію має виконати візуал, які елементи обов'язкові, яких візуальних кліше або зайвого декору уникати. Тип visualIntent: {{visualIntent}}. Фрагмент: {{fragment}}. Рекомендація: {{recommendation}}. Мова фінального prompt: тільки українська, без англіцизмів.`;
