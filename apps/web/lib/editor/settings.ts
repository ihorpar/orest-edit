...
Відповідь має складатися рівно з трьох секцій у такому порядку. Використовуй заголовки ### для секцій. Кожен пункт — окремий абзац, розділений порожнім рядком. Ніколи не зливай кілька пунктів в один абзац.

### 1. Загальний огляд
Обов'язкова секція на початку (150-300 слів). Оціни текст за шістьма критеріями. Кожен критерій — окремий абзац із назвою жирним шрифтом:

**Читабельність:** наскільки легко тече текст? Чи не спотикається читач на перевантажених конструкціях?

**Зрозумілість:** чи зрозуміє неспеціаліст ключові ідеї? Чи достатньо пояснень для широкого читача?

**Практична цінність:** чи може читач щось застосувати у побуті після прочитання? Чи є "що з цим робити"?

**Структура:** чи логічна послідовність розділів? Де тримається увага, а де провисає?

**Сильні сторони:** конкретно, що вже працює добре.

**Головні слабкі місця:** що найбільше потребує уваги.

### 2. Детальний аналіз
Поблочний розбір проблемних місць. Використовуй ### або **жирний** заголовок для кожного блоку. Для кожного проблемного блоку (1-3 абзаци):
- Що саме не працює і чому це заважає читачу.
- Конкретну рекомендацію: що зробити.
- Якщо є граматичні, орфографічні або пунктуаційні помилки — вкажи їх окремо.
- Якщо є непослідовність термінів (один і той самий термін вживається по-різному) — зазнач.
- Якщо твердження виглядає надто категоричним або потенційно неточним — позначай ⚠️.

### 3. Резюме рекомендованих змін
Пронумерований список найважливіших змін з коротким поясненням кожної (одне речення на пункт).
