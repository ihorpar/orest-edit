...
export const DEFAULT_EXPERTISE_PROMPT = appendBulletListPunctuationRule(`Працюй у режимі макродіагностики великого розділу як суворий книжковий редактор.

Не заспокоюй автора, не починай із похвали, не відкривай відповідь фразами на кшталт «ось аналіз» або «нижче подано діагностику». Кожне критичне зауваження має спиратися на конкретні абзаци у форматі «абз. NNN».

Фокусуйся на архітектурі розділу, читацькому маршруті, дублюванні, провисанні логіки та зайвих бокових блоках. Не підміняй структурний аналіз мікростилістичними заувагами. Не повторюй одну й ту саму проблему різними словами.

Формат відповіді: Markdown українською. Дотримуйся обов'язкових заголовків і рубрики з системних інструкцій цього запуску (режим глибини діагностики). Починай відповідь відразу з «## Головний діагноз розділу».
...
export const DEFAULT_WORKFLOW_STEP_PROMPTS: Record<EditorialReviewStepId, string> = {
  diagnostics:
    "Зроби глибоку редакторську діагностику рукопису: вердикт, системні проблеми, читацький бар'єр і пріоритетний план. Режим глибини (по суті / розширена) задає обсяг секцій. Це review-only крок, без карток дій.",
  fact_check:
...
function sanitizeExpertisePrompt(candidate: string | null | undefined, fallback: string): string {
  ...
  // Drop persisted pre-depth-mode templates that still demand the old 7-section macro pack.
  if (
    /##\s*Карта розділу/i.test(trimmed) ||
    /##\s*Показові абзаци/i.test(trimmed) ||
    /##\s*Section map/i.test(trimmed) ||
    /##\s*Exemplar paragraphs/i.test(trimmed) ||
    /8-15\s+найпоказовіших/i.test(trimmed) ||
    /8-15\s+of the most telling/i.test(trimmed)
  ) {
    return fallback;
  }
...
export function sanitizeEditorSettings(candidate: Partial<EditorSettings> | null | undefined, locale: AppLocale = "uk"): EditorSettings {
  ...
  return {
    ...
    expertisePrompt: sanitizeExpertisePrompt(
      typeof candidate?.expertisePrompt === "string" ? candidate.expertisePrompt : null,
      defaults.expertisePrompt
    ),
    cardsPrompt:
