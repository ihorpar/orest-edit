...
test("generateEditorialReview collapses provider block ranges to a single anchor block", async () => {
  const response = await generateEditorialReview(createRequest({ apiKey: "test-key" }), {
    fetchImpl: async () =>
      new Response(
        JSON.stringify({
          output_text: JSON.stringify({
            items: [
              {
                title: "Звузити фокус правки",
                reason: "Рекомендація має лишатися локальною.",
                recommendation: "Переписати лише цільовий абзац.",
                recommendationType: "rewrite",
                suggestedAction: "rewrite_text",
                priority: "high",
                blockStart: 0,
  ...
  assert.equal(response.items.length, 1);
  assert.deepEqual(response.items[0]?.anchor.blockIds, ["p1"]);
  assert.deepEqual(response.items[0]?.anchor.generationBlockRange, { start: 1, end: 1 });
});
