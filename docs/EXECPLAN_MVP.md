...
- [x] (2026-04-02 00:00Z) Upgraded the compare dialog so the `Стало` column can edit live text/list/callout blocks and write those manual fixes directly back into the manuscript draft with normal undo history.
...
The 2026-04-02 compare-dialog follow-up turned that snapshot view into a lightweight repair surface. When the compared blocks still exist in the live manuscript and are text-compatible, the `Стало` column now renders editable block fields instead of frozen text, and every change writes back into the draft through the same manual-edit history path used by the editor. This preserves undo/redo behavior while letting an editor fine-tune an accepted AI change without leaving the compare context.
