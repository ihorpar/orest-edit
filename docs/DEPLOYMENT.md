...
- `POST /api/edit/review` with `async: false` keeps the direct synchronous path for debugging and tests; recommendation-card steps still chunk in waves of 3 on that path
...
- Recommendation-card steps (`clarity`, `structure`, `interest`, `visuals`, `formatting`, `emphasis`) execute one durable provider request per planned 16k character chunk, in document-order waves of 3. Retryable timeout/network/429 errors retry that chunk; after retries a hole is recorded and later chunks continue. Missing API key / 401 / 403 remain whole-run fatal. `diagnostics` and `fact_check` stay one-shot whole-document and can still abort at 280s on large chapters
- `final_editing` runs one durable plan step, then **one** durable generate step for the whole plan (not one step per card). Empty custom prompt and empty validated plans fail loud. Missing cards can be retried as a single `customRequestPlanAction` without re-planning. Exact card counts from free text are not guaranteed without a future UI count field; the plan prompt aims near stated totals with a hard ceiling of 20
- GET `/api/edit/review?runId=...` must return quickly while the workflow is running: it reads already-written namespaced stream chunks (`getTailIndex`) instead of waiting for stream close. A blocking poll previously occupied the 300s route until `FUNCTION_INVOCATION_TIMEOUT`
- DELETE `/api/edit/review?runId=...` requires the same session and capability header as GET, then cancels the workflow run. The editor Stop control uses this; poll/JSON/platform failures also bury the local run so reload cannot resume a hung GET
- The browser poller aborts a single GET after 12 seconds and treats Vercel HTML timeout bodies as a fail-loud platform error, never `Unexpected token 'A'`
