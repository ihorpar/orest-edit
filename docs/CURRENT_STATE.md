...
- The manuscript formatting toolbar uses a centered content-width surface: undo/redo/change-history, inline/block formatting, and insertion controls all live in the sticky toolbar; tablet layouts keep the full toolbar in one visible row with compact but touch-comfortable button targets and no horizontal clipping, the normal-text control uses a compact `Т` while H1/H2/H3 remain visually larger in the hierarchy, and a self-healing fixed-position rescue mode can restore the toolbar automatically or manually via `Cmd/Ctrl+.` or a touch rescue button when iPad/Safari sticky positioning drifts
