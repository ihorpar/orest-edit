...
- Structure meta line includes «Вставити всі» once ready proposed subheads exist; it inserts every ready title in document order in one undo step
- Recommendation steps no longer show the green «Підготовлено рекомендації…» status strip; Structure also dropped the redundant «План розділу» section header above the tree
...
- English edition localization ExecPlan **complete** (M1–M9): full settings/editor UI copy, server prompts (`patch.ts`, `review.ts`, `review-action.ts`), locale-scoped storage, spellcheck `en-US`/`uk-UA`, API validation/provider error messages, fact-check linked cards + structure outline labels, 221 passing tests
...
- The manuscript formatting toolbar uses a centered content-width surface: undo/redo/change-history, inline/block formatting, and insertion controls all live in the sticky toolbar; tablet layouts keep the toolbar in a compact single row with touch horizontal scrolling instead of clipping, the normal-text control uses a compact `Т` while H1/H2/H3 remain visually larger in the hierarchy, and a self-healing fixed-position rescue mode restores the toolbar below the app top bar automatically or manually via `Cmd/Ctrl+.` or a touch rescue button when iPad/Safari sticky positioning drifts
...
- Long workflow reviews now use a 300-second Vercel route duration with an internal provider abort below the platform limit. GET `/api/edit/review?runId=...` reads already-written workflow stream chunks via `getTailIndex()` and must not wait for the run to finish; a hanging poll previously surfaced as `FUNCTION_INVOCATION_TIMEOUT` / `Unexpected token 'A'`
- Browser review polls abort after a base 12 seconds (plus a small stretch for large chapters) and wait at least 1 second between GETs, stretching to 3 seconds on ~160k-character chapters. In-flight `kind: "run"` GETs use an `afterItem` cursor so each poll returns only newly streamed cards (`items` delta) plus `itemCursor`/`itemCount`; the client persists that cursor on the active run for resume. Platform HTML, JSON parse failures, and other poll errors bury the persisted run so reload cannot resurrect it. `Зупинити` cancels the workflow via DELETE `/api/edit/review?runId=...` without wiping the manuscript (`Очистити` still does that)
...
- Starting a workflow step never switches the visible drawer to another step. A zombie non-terminal run from a failed poll is buried; a genuinely in-flight other step blocks Start with a wait message. Chunk progress and fragment retries render only for the run's own step. Completed workflow results without a valid `diagnostics` object fail loud with a localized message instead of leaking `Cannot read properties of undefined (reading 'requestId')`. While a step is running, the primary action becomes `Зупинити` / Stop.
- Recommendation-card steps (`structure`, `clarity`, `interest`, `visuals`, `formatting`, `emphasis`) plan character-budget chunks at 12,000-16,000 source characters with a 400-block safety cap. The representative 142,870-character fixture still produces about 10 provider chunks. Chunks run in document-order waves of 3. Workflow progress stream writes (`getWritable().getWriter()`) run only inside `"use step"` functions; writing them from the `"use workflow"` orchestrator fails immediately with `Not supported in workflow functions`. Focused-step review prompts omit the shared mixed-type `cardsPrompt` catalog; callout-prepare drafting rules stay on proposal generation. Cards from finished fragments stream on GET `kind: "run"`; a failed chunk after retries is a hole and later chunks continue. Provider abort surfaces a localized timeout, never `This operation was aborted`
- `final_editing` / `Власний запит` uses chapter-level plan → generate: one plan call over the **full manuscript** (every block with абз. label + `[blockId]`; hard ceiling 20 actions, no regex count parsing), then **one** card-generate call for the whole plan (local slices around planned anchors, not per-action durable steps). Progress phases are `planning` then `generating`. Failed generate is a hole set; a single planned action can still be retried with `customRequestPlanAction` in preserve mode.
...
- The old visible `Фінальна редактура` stage is replaced by `Власний запит`; it requires a non-empty editor instruction (fail-loud without it), does not depend on diagnostics (and does not inject diagnostics context into the prompt), always uses `runMode: replace` for full re-runs so prior cards for that step are replaced, and can generate executable recommendation card types (not emphasis accents) while keeping the internal `final_editing` id for draft/history compatibility. Absolute counts in free text are handled by the chapter plan prompt + hard ceiling 20, then one generate call for the whole plan — not by regex quotas, per-fragment 0–2 caps, or one durable step per card.
...
- Provider model presets (2026-07-22): OpenAI `gpt-5.6-sol` / `gpt-5.6-luna` / `gpt-5.6-luna-low` with Responses `reasoning.effort`; Gemini `gemini-3.7-flash` / `gemini-3.5-flash-lite` with `thinkingConfig.thinkingLevel=high`; retired Pro/legacy ids remap via `normalizeModelId` (including `gemini-3.6-flash` → `gemini-3.7-flash`)
...
- Compact recommendation cards now truncate recommendation copy to two lines by default. Expanding a card keeps actions visible and clamps the recommendation to four lines, with a grey `Більше…` / `Менше` control for longer copy instead of growing the card without bound.
...
- The local composer no longer shows a separate drag-handle chip in the top row; drag still works from the top strip, and the `Правка` footer now uses shorter intent labels (`Список`, `Підзаголовок`) plus a non-wrapping scrollable control row so the send button stays visible on compact widths
...
- Runtime smoke on 2026-04-16 against `http://127.0.0.1:3000/editor` after API login confirmed selecting a paragraph opens the floating `Правка` footer with the subsection intent, no `Таблиця`, no framework overlay, and no browser console/page errors.
...
- Runtime QA on 2026-04-16 against production `next start` at `http://127.0.0.1:3102` confirmed: toolbar layout stays single-row and unclipped at `960`, `1024`, `1180`, `1290`, `1366`, and `1401`; `Ctrl+M` forces manual toolbar rescue into visible fixed positioning; an artificially detached toolbar is caught by automatic rescue and restored visible inside the editor shell
