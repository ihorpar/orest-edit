...
- Browser review polls abort after 12 seconds. Platform HTML, JSON parse failures, and other poll errors bury the persisted run so reload cannot resurrect it. `Зупинити` cancels the workflow via DELETE `/api/edit/review?runId=...` without wiping the manuscript (`Очистити` still does that)
