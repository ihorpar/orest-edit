...
- Browser review polls abort after 12 seconds and wait at least 1 second between GETs, stretching to 3 seconds on ~160k-character chapters. Platform HTML, JSON parse failures, and other poll errors bury the persisted run so reload cannot resurrect it. `Зупинити` cancels the workflow via DELETE `/api/edit/review?runId=...` without wiping the manuscript (`Очистити` still does that)
